...
const { WHITES, LP, RP, STRING, NUMBER, WORD, COMMA, NEWLINE } = require("./tokenRegex.js");
...
function parseExpression() {
  ...
  if (lookahead.type == "STRING") {
    let string = new Value(lookahead.value);
    lex();
    expr= parseApply(string);
    return expr;
  } else if (lookahead.type == "NUMBER") {
    let number= new Value(lookahead.value);
    lex();
    expr= parseApply(number);
    return expr;
  } else if (lookahead.type == "WORD") {
    let word= new Word(lookahead.value);
    lex();
...
function parseApply(tree) {
  ...
  if (lookahead.type !== "LP") return tree; 
